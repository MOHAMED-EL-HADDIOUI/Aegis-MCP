# Aegis-MCP developer workflows. POSIX-sh compatible; recipes use cargo
# (and docker/node for container/dashboard targets) so they also run on Windows
# via Git Bash / PowerShell `make`. Run `make help` for the list.
BIND ?= 127.0.0.1:8787

.PHONY: help dev build test security fuzz benchmark lint format docker compose-up dashboard serve clean
...
# AEGIS_AUDIT_DB=":memory:" keeps the demo side-effect free (fresh empty chain).
security:
	cargo test --workspace
